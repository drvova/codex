...
fn canonical_mcp_server_key(name: &str, config: &McpServerConfig) -> String {
    match &config.transport {
        ...
        McpServerTransportConfig::Sse { sse_url, .. } => canonical_mcp_key("sse", sse_url, name),
    }
}
...
fn canonical_mcp_dependency_key(dependency: &SkillToolDependency) -> Result<String, String> {
    ...
    if transport.eq_ignore_ascii_case("sse") {
        let url = dependency
            .url
            .as_ref()
            .ok_or_else(|| "missing url for sse dependency".to_string())?;
        return Ok(canonical_mcp_key("sse", url, &dependency.value));
    }
...
fn mcp_dependency_to_server_config(
    ...
    if transport.eq_ignore_ascii_case("sse") {
        let url = dependency
            ...
            .ok_or_else(|| "missing url for sse dependency".to_string())?;
        return Ok(McpServerConfig {
            transport: McpServerTransportConfig::Sse {
                sse_url: url.clone(),
                message_url: None,
                bearer_token_env_var: None,
                http_headers: None,
